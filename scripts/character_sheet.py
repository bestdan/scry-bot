...
class CharacterSheet:
            ...
            if armor_type_id == 4:  # Shield
                ...
                for mod in item_def.get("grantedModifiers", []):
                    if (
                        ...
                        and mod.get("type") == "bonus"
                    ):
                        shield_bonus += (
                            mod.get("value", 0) or mod.get("fixedValue", 0) or 0
                        )
    ...
    def display(self):
        ...
        print("─" * 55)
        print(
            f"  AC: {self.ac}    HP: {self.current_hp}/{self.max_hp}    Speed: {self.speed} ft"
        )
        print(
            f"  Initiative: {self.initiative:+d}    Proficiency Bonus: +{self.proficiency_bonus}"
        )
...
def show_overview(data: dict):
    ...
    classes = ", ".join(
        [f"{c['definition']['name']} {c['level']}" for c in data.get("classes", [])]
    )

    print(f"{name} - {race} {classes}")
    print(
        f"Level {sheet.total_level} | HP: {sheet.current_hp}/{sheet.max_hp} | AC: {sheet.ac}"
    )
    print()
    print("Abilities:", end=" ")
    for stat in ["STR", "DEX", "CON", "INT", "WIS", "CHA"]:
        print(
            f"{stat} {sheet.abilities[stat]['total']}({sheet.abilities[stat]['mod']:+d})",
            end="  ",
        )
    print()
    ...
    if sheet.spell_ability:
        print(
            f"Spellcasting: {sheet.spell_ability} | Attack +{sheet.spell_attack} | DC {sheet.spell_dc}"
        )
...
def show_summary(data: dict):
    ...
    print(
        f"{name}: Level {sheet.total_level} {race} {classes} | HP {sheet.current_hp}/{sheet.max_hp} | AC {sheet.ac}"
    )
...
def main():
    ...
    if len(args) < 1:
        print(
            "Usage: python3 character_sheet.py [--dir DIR] <command> <character_name>"
        )
        print(
            "       python3 character_sheet.py [--dir DIR] <character_name>  (defaults to 'sheet')"
        )
        print("\nCommands:")
